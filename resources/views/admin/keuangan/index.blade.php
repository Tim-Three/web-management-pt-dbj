...
@section('sidebar-menu')
    ...
    <a href="{{ route('admin.keuangan.index') }}" class="flex items-center gap-3 px-2 py-2 rounded-lg bg-green-50 text-green-700 font-medium text-sm mb-1">
...
<div class="grid grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-2xl p-5 border border-gray-100">
        ...
        <p class="text-xl font-bold text-green-600">Rp{{ number_format($keuanganBulanIni?->total_pendapatan ?? 0, 0, ',', '.') }}</p>
    </div>
    <div class="bg-white rounded-2xl p-5 border border-gray-100">
        <p class="text-xs text-gray-400 mb-2">Total pengeluaran di bulan ini</p>
        <p class="text-xl font-bold text-red-500">Rp{{ number_format($keuanganBulanIni?->total_pengeluaran ?? 0, 0, ',', '.') }}</p>
    </div>
    <div class="bg-white rounded-2xl p-5 border border-gray-100">
        <p class="text-xs text-gray-400 mb-2">Total keuntungan di bulan ini</p>
        <p class="text-xl font-bold text-green-600">Rp{{ number_format($keuanganBulanIni?->keuntungan ?? 0, 0, ',', '.') }}</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl p-6 border border-gray-100">
    ...
    <form method="POST" action="{{ route('admin.keuangan.store') }}">
        ...
        <button type="submit" class="px-6 py-2.5 bg-green-600 text-white rounded-xl text-sm font-semibold hover:bg-green-700 transition">
