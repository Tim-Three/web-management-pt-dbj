...
@section('content')
    ...
    <div class="grid gap-4 mb-4" style="grid-template-columns: 300px 1fr;">

        {{-- Kiri: Total Karyawan + Detail Kehadiran (stacked) --}}
        <div class="flex flex-col gap-4">

            {{-- Total Karyawan Card (hijau) --}}
            <div class="bg-green-600 rounded-2xl p-5 flex items-start justify-between">
                <div>
                    <p class="text-green-200 text-xs mb-3">Total karyawan</p>
                    <p class="text-5xl font-bold text-white">{{ $totalKaryawan }}</p>
                </div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-gray-100 flex items-start justify-between">
                <div class="flex-1">
                    <p class="text-xs text-gray-400 mb-4">Detail kehadiran hari ini</p>
                    <div class="flex justify-start gap-10">
                        ...
                        <div>
                            <p class="text-3xl font-bold text-yellow-400">{{ $telat }}</p>
                            <p class="text-xs text-gray-400 mt-1">Telat</p>
                        </div>
                        <div>
                            <p class="text-3xl font-bold text-blue-400">{{ $izin }}</p>
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100">
            <div class="flex items-center justify-between mb-4">
                <p class="text-sm font-semibold text-gray-700">Kehadiran terkini</p>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <img src="{{ $k->foto ? Storage::url($k->foto) : 'https://ui-avatars.com/api/?name=' . urlencode($k->name) . '&background=6366f1&color=fff&size=64' }}"
                                class="w-10 h-10 rounded-full object-cover" id="preview-edit">
                            <div>
                                <div class="flex items-center gap-2">
                                    <p class="text-sm font-medium text-gray-800">{{ $k->user->name }}</p>
                                    <span
                                        class="px-2 py-0.5 rounded-full text-xs font-medium {{ $k->status === 'telat' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600' }}">
                                        {{ $k->status === 'telat' ? 'Telat hadir' : 'Tepat waktu' }}
                                    </span>
                                </div>
                                <p class="text-xs text-gray-400">{{ $k->user->posisi ?? 'Karyawan' }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-semibold text-gray-700">
                                {{ \Carbon\Carbon::parse($k->jam_masuk)->format('h:i A') }}</p>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-400 text-xs">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left font-medium">Posisi</th>
                    <th class="px-6 py-3 text-left font-medium">Email</th>
                    <th class="px-6 py-3 text-left font-medium">No.Telp</th>
                    <th class="px-6 py-3 text-left font-medium">Domisili</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs font-medium {{ $statusBadge[0] }}">{{ $statusBadge[1] }}</span>
                                    </div>
                                    <p class="text-xs text-gray-400">{{ $k->nip ?? '-' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-3 text-gray-400">{{ $k->posisi ?? '-' }}</td>
                        <td class="px-6 py-3 text-gray-400">{{ $k->email }}</td>
                        <td class="px-6 py-3 text-gray-400">{{ $k->no_telp ?? '-' }}</td>
                        <td class="px-6 py-3 text-gray-400">{{ $k->domisili ?? '-' }}</td>
                        <td class="px-6 py-3 relative">
                            ...
                            <div id="dropdown-{{ $k->id }}"
                                class="hidden absolute right-6 top-10 bg-white border border-gray-100 rounded-xl shadow-lg z-10 w-36 py-1">
                                ...
                                <form method="POST" action="{{ route('admin.karyawan.destroy', $k) }}"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="flex items-center gap-2 px-4 py-2 text-sm text-red-400 hover:bg-red-50 w-full text-left">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
