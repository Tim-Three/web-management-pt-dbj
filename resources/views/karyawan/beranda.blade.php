...
@section('page-title', 'Haii, selamat datang ' . auth()->user()->name . '!')

@section('sidebar-menu')
    <a href="{{ route('karyawan.beranda') }}" class="flex items-center gap-3 px-2 py-2 rounded-lg bg-green-50 text-green-700 font-medium text-sm mb-1">
    ...
    <div class="grid grid-cols-2 gap-3">
        <form method="POST" action="{{ route('karyawan.absen.masuk') }}">
            ...
            <button type="submit"
                ...
                class="w-full py-4 rounded-xl font-semibold text-sm {{ $absensiHariIni ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700' }} transition">
                Absen masuk
            </button>
        </form>
        <form method="POST" action="{{ route('karyawan.absen.pulang') }}">
            ...
            <button type="submit"
                ...
                class="w-full py-4 rounded-xl font-semibold text-sm {{ (!$absensiHariIni || $absensiHariIni->jam_pulang) ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-700 text-white hover:bg-gray-800' }} transition">
                Absen pulang
            </button>
        </form>
    </div>
...
<div class="bg-white rounded-2xl p-5 border border-gray-100 mb-6">
    ...
    <form method="POST" action="{{ route('karyawan.cuti.ajukan') }}">
        ...
        <div class="flex items-center gap-3 mb-4">
            ...
            <div class="flex-1">
                ...
                <input type="date" name="sampai" value="{{ old('sampai') }}"
                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-green-500">
                @error('sampai')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
        </div>
        <div class="mb-4">
        ...
        <button type="submit" class="w-full py-3.5 bg-green-600 text-white rounded-xl font-semibold text-sm hover:bg-green-700 transition">
            Ajukan cuti
        </button>
    </form>
</div>
...
<div class="grid grid-cols-2 gap-4">
    ...
    <div class="bg-white rounded-2xl p-5 border border-gray-100">
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-3 text-sm items-center">
                <span class="text-gray-600 text-xs">{{ $absen->tanggal->locale('id')->isoFormat('dddd DD/MM/YYYY') }}</span>
                <span class="text-center text-green-600 font-medium">{{ $absen->jam_masuk ? \Carbon\Carbon::parse($absen->jam_masuk)->format('H:i') : '-' }}</span>
                <span class="text-center text-red-500 font-medium">{{ $absen->jam_pulang ? \Carbon\Carbon::parse($absen->jam_pulang)->format('H:i') : '-' }}</span>
            </div>
            @empty
    ...
    <div class="bg-white rounded-2xl p-5 border border-gray-100">
        ...
        <div class="space-y-3">
            <div class="grid grid-cols-3 text-xs text-gray-400 border-b border-gray-50 pb-2">
                <span>Tanggal pengajuan</span><span class="text-center">Dari</span><span class="text-center">Sampai</span>
            </div>
